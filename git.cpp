...
const QString Git::getLaneParent(const QString &fromSHA, int laneNum)
{
   ...
   for (int idx = rs.orderIdx - 1; idx >= 0; --idx)
   {
      ...
      if (!isFreeLane(r.lanes[laneNum]))
      {
         ...
         auto type = r.lanes[laneNum], parNum = 0;
...
const RevFile *Git::getAllMergeFiles(const QString &sha)
{

   const QString &mySha(ALL_MERGE_FILES + QString(sha));
   if (mRevsFiles.contains(mySha))
      return mRevsFiles[mySha];

   QString runCmd(QString("git diff-tree --no-color -r -m ").arg(sha)), runOutput;
...
const Revision Git::fakeWorkDirRev(const QString &parent, const QString &log, const QString &longLog, int idx)
{
   ...
   auto c = fakeRevData(ZERO_SHA, parents, author, date, log, longLog, patch, idx);
   c.isDiffCache = true;
...
void Git::getDiffIndex()
{
   ...
   const auto r = fakeWorkDirRev(head, log, status, mRevCache->revOrderCount());
...
bool Git::startParseProc(const QStringList &initCmd)
{
   ...
   connect(dl, &DataLoader::loaded, this, &Git::loadCompleted);
...
void Git::updateDescMap(const Revision &r, uint idx, QHash<QPair<uint, uint>, bool> &dm, QHash<uint, QVector<int>> &dv)
{
   ...
   if (r.descRefsMaster != -1)
   {
      ...
      for (int i = 0; i < tmp.descRefs.count(); i++)
      {

         const auto descendentRefIndex = static_cast<uint>(tmp.descRefs.at(i));
         if (!dv.contains(descendentRefIndex))
            return;

         const QVector<int> &dvv = dv.value(descendentRefIndex);
...
void Git::mergeNearTags(bool down, Revision &p, const Revision &r, const QHash<QPair<uint, uint>, bool> &dm)
{
   ...
   for (int s2 = 0; s2 < src2.count(); s2++)
   {
      ...
      for (int s1 = 0; s1 < src1.count(); s1++)
      {
         ...
         const auto key = qMakePair(static_cast<uint>(src2[s2]), static_cast<uint>(src1[s1]));
...
// TODO: Insert the new modified revision
void Git::indexTree()
{
   ...
   for (int i = 0, cnt = mRevCache->revOrderCount(); i < cnt; ++i)
   {
      ...
      if (isT)
      {
         updateDescMap(r, static_cast<uint>(i), descMap, descVect);
         r.descRefs.clear();
         r.descRefs.append(i);
      }

      mRevCache->insertRevision(r.sha(), r);

      for (int y = 0; y < static_cast<int>(r.parentsCount()); ++y)
      {
